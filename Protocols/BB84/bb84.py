...
for i in range(num_iters):
    ...
    sender_basis[i] = '+' if counts_arr[3] == 0 else 'X'
    sender_bits[i] = counts_arr[1]
    receiver_basis[i] = '+' if counts_arr[2] == 0 else 'X'
    receiver_bits[i] = counts_arr[0]

rke = (sender_basis == receiver_basis).mean()
key = sender_bits[np.where((sender_basis == receiver_basis) == True)[0]]

print(sender_basis[:10])
print(sender_bits[:10])
print(receiver_basis[:10])
print(receiver_bits[:10])
print(rke)
print(key)
